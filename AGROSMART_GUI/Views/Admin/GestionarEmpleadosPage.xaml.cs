        ...
        private void BtnEliminar_Click(object sender, RoutedEventArgs e)
        {
            if ((sender as FrameworkElement)?.Tag is EmpleadoViewModel emp)
            {
                ...
                if (result == MessageBoxResult.Yes)
                {
                    try
                    {
                        // Primero obtener las entidades completas
                        var empleado = _empleadoService.ObtenerPorId(emp.IdUsuario);
                        var usuario = _usuarioService.ObtenerPorId(emp.IdUsuario);

                        if (empleado == null || usuario == null)
                        {
                            MessageBox.Show("No se encontró el empleado o usuario.", "Error",
                                MessageBoxButton.OK, MessageBoxImage.Error);
                            return;
                        }

                        // Eliminar empleado (recibe la entidad completa)
                        bool eliminadoEmp = _empleadoService.Eliminar(empleado);

                        if (eliminadoEmp)
                        {
                            // Eliminar usuario (recibe la entidad completa)
                            ...
                            {
                                MessageBox.Show("El empleado fue eliminado pero hubo un problema al eliminar el usuario.", "Advertencia",
